# Follow https://github.com/tzt101/MichiGAN
import torch
...
# parser.add_argument('--image_path', type=str, default="/root/autodl-tmp/tzz/dataset/p3m-10k/validation/P3M-500-NP/image", help='Path to image')
# parser.add_argument('--hairmask_path',type=str, default=r"/root/autodl-tmp/tzz/dataset/p3m-10k/validation/P3M-500-NP/trimap", help='Path to hair mask')
# parser.add_argument('--orientation_root', type=str, default="/root/autodl-tmp/tzz/dataset/p3m-10k/validation/P3M-500-NP/orien1_color", help='Root to save hair orientation map')

# parser.add_argument('--image_path', type=str, default="/root/autodl-tmp/tzz/dataset/p3m-10k/train/blurred_image", help='Path to image')
# parser.add_argument('--hairmask_path',type=str, default=r"/root/autodl-tmp/tzz/dataset/p3m-10k/train_hair/blurred_hair", help='Path to hair mask')
# parser.add_argument('--orientation_root', type=str, default="/root/autodl-tmp/tzz/dataset/p3m-10k/train_hair/orientation_sam3", help='Root to save hair orientation map')

parser.add_argument('--image_path', type=str, default="/root/autodl-tmp/tzz/dataset/p3m-10k/validation/P3M-500-P/image", help='Path to image')
parser.add_argument('--hairmask_path',type=str, default=r"/root/autodl-tmp/tzz/dataset/p3m-10k/validation/P3M-500-P/hair", help='Path to hair mask')
parser.add_argument('--orientation_root', type=str, default="/root/autodl-tmp/tzz/dataset/p3m-10k/validation/P3M-500-P/orientation", help='Root to save hair orientation map')
...
if __name__ == '__main__':
    ...
    for file in tqdm(os.listdir(args.image_path)):
        ...
        output_file = os.path.join(args.orientation_root, os.path.splitext(file)[0] + '.jpg')
        if os.path.exists(output_file):
            continue  
        
        image_path = file_path
        hair_path = os.path.join(args.hairmask_path, os.path.splitext(file)[0] + '.jpg')

        image = Image.open(image_path)
        mask = np.array(Image.open(hair_path).convert('RGB'))
        mask = cv2.cvtColor(mask, cv2.COLOR_RGB2GRAY)
        # mask = ((mask >= 100) & (mask <= 150)) * 1
        mask = ((mask >= 100) & (mask <= 255)) * 1  # foreground+unknown

        mask_dilated = cv2.dilate(mask.astype(np.uint8), dilate_kernel, iterations=1)
        mask_dilated = (mask_dilated > 0).astype(np.uint8)
        ...
        mask_tensor = torch.from_numpy(mask_dilated).float()
        ...
        orient_save.save(os.path.join(args.orientation_root, os.path.basename(file).split('jpg')[0]+'png'))
